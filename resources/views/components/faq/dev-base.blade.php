    ...
    <div class="faq_question">
        <div class="flex items-center text-sm px-2 py-4" style="max-height: 56px;">
            <div class="float-left flex">
                <div class="text-tm-c-0D5E49 PtSerif italic font-bold text-lg">
                    {{$questionTitle}}&nbsp;
                </div>
                <div class="AppSdGothicNeoR font-bold text-lg leading-snug">{{$question}}</div>
            </div>
            <div class="float-left ml-auto cursor-pointer">
                <img class="faq_ic {{$now}} w-8 h-8" src="https://d2pyzcqibfhr70.cloudfront.net/resource/icons/pc-ic-dropdown.png" alt="">
            </div>
        </div>
    </div>
    ...
    @if($now==='true')
        <div class="faq_answer {{$now}}">
            ...
            <div>
                ...
                <span class="faq_answer_text AppSdGothicNeoR text-lg leading-snug">{!! $answer !!}</span>
                </div>
            </div>
        </div>
        @else
...
<script>
    var $faq_question = $('.faq_question').off().on('click', function () {
        ...
        GA_event('FAQ_click',[$('.faq_answer:eq(' + index + ') .faq_answer_text').text(),index]);
