<div wire:init="itemLoad">
    ...
    <div class="pt-6">
        <div>
            ...
            <div class="border-t-2 border-b-2 border-solid border-white"
                 @if(!$edit) x-data x-on:click="$dispatch('notification', {text: '수정하기 버튼을 누른 후에 수정이 가능합니다.', time : '4000', type : 'wbc-mtc-text-30373F-bg-d7d3cf'})" @endif>
                ...
                <div class="flex items-center border-b border-solid border-tm-c-979b9f border-opacity-50">
                    <div class="flex-0 px-4 h-full">
                        <label for="method.1" class="select-none">
                            <input type="radio" id="method.1" wire:model="method" @if(!$edit) disabled @endif value="입금가" class="hidden">
                ...
                <div class="flex items-center">
                    <div class="flex-0 px-4 h-full">
                        <label for="method.0" class="select-none">
                            <input type="radio" id="method.0" wire:model="method" @if(!$edit) disabled @endif value="수수료" class="hidden">
                            <div class="flex items-center space-x-2 AppSdGothicNeoR font-bold text-sm text-white">
                                <div>
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6" viewBox="0 0 24 24">
                                        @if($method === '수수료')
            ...
            @foreach (\App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereTarget('method')->whereNull('status')->get('content') as $item)
    ...
    <div class="pt-6">
                        ...
                        <div class="flex-0 @if($editing) grid grid-cols-3 @endif">
                            ...
                            <div class="@if($editing) col-span-2 @endif flex items-center justify-center h-14 sm:h-16 @if($errors->has('periods_value.'.$index))ring-2 ring-tm-c-ff7777 @endif">
                                <div class="w-full AppSdGothicNeoR font-bold text-tm-c-30373F text-xs 2xs:text-base sm:text-lg">
                                    <input type="text" wire:model.lazy="periods_value.{{$index}}" id="periods_value.{{$index}}" maxlength="15" @if($index <= 3) readonly @endif
                                    ...
                                    <div>
                                        @if($method === '입금가')
                                            <div>
                                                <div class="w-full bg-white rounded-sm">
                                                    <div class="h-full flex AppSdGothicNeoR text-tm-c-30373F @if($errors->has('prices.'.$index.'.'.$i))ring-2 ring-tm-c-ff7777 rounded-sm @endif @if(\App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereNull('status')->whereIn('target',['prices.'.$index.'.'.$i])->count()>=1)ring-2 ring-tm-c-ff7777 rounded-sm @endif">
                                                        <div class="#px-3"
                                                ...
                                                @foreach (\App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereTarget('prices.'.$index.'.'.$i)->whereNull('status')->get('content') as $item)
                                                    <div class="mt-2 text-tm-c-da5542">
                                                        <p class="whitespace-pre-line AppSdGothicNeoR leading-2 text-sm">{!! $item->content !!}</p>
                                                    </div>
                                                @endforeach
                                                @if(\App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereNull('status')
                                                        ->whereIn('target',['prices.'.$index.'.'.$i])->count()
                                                        !== collect($this->count[$i] ?? 0)->max())
                                                    @for ($f = 0; $f < (collect($this->count[$i] - \App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereNull('status')
                                                        ->whereIn('target',['prices.'.$index.'.'.$i])->count()) ?? 0)->max(); $f++)
                                        ...
                                        @elseif($method === '수수료')
                                            <div>
                                                ...
                                                @foreach (\App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereNull('status')->whereIn('target',['sale_prices.'.$index.'.'.$i,'fees.'.$index.'.'.$i])->get('content') as $item)
                                                    <div class="mt-2 text-tm-c-da5542">
                                                        <p class="whitespace-pre-line AppSdGothicNeoR leading-2 text-sm">{!! $item->content !!}</p>
                                                    </div>
                                                @endforeach
                                                @if(\App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereNull('status')
                                                        ->whereIn('target',['sale_prices.'.$index.'.'.$i,'fees.'.$index.'.'.$i])->count()
                                                        !== collect($this->count[$i] ?? 0)->max())
                                                    @for ($f = 0; $f < (collect($this->count[$i] - \App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereNull('status')
                                                        ->whereIn('target',['sale_prices.'.$index.'.'.$i,'fees.'.$index.'.'.$i])->count()) ?? 0)->max(); $f++)
                                                        <div class="mt-2 select-none">
                                                            <p class="whitespace-pre-line text-sm leading-2">&nbsp;</p>
                                                        </div>
                                                    @endfor
        ...
        @if($period_count <8)
            <div class="">
                <div class="h-full">
                    <div class="h-full border border-dashed @if(\App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereTarget('periodCount')->whereNull('status')->count()>=1) border-tm-c-da5542 @else border-white @endif flex flex-wrap items-center justify-center cursor-pointer"
                         @if($edit) wire:click="periodCount('add')" @else x-data x-on:click="$dispatch('notification', {text: '수정하기 버튼을 누른 후에 수정이 가능합니다.', time : '4000', type : 'wbc-mtc-text-30373F-bg-d7d3cf'})" @endif
                ...
                @foreach (\App\AddHotelNeedToModify::whereAddHotelId($addHotel->id)->whereModel('AddHotelItem')->whereTarget('periodCount')->whereNull('status')->get('content') as $item)
                    <div class="mt-2 text-tm-c-da5542">
                        <p class="whitespace-pre-line AppSdGothicNeoR leading-normal text-sm break-all">{!! $item->content !!}</p>
                    </div>
                @endforeach
