...
<div>
    <div class="datatable px-2" x-data="datatables()">
        <div class="flex flex-wrap items-center gap-2 py-2">
            ...
            <div class="flex items-center">
                <select wire:model.lazy="searchData.flitter"
                        wire:input="searchDataSave"
                        class="w-40 shadow border rounded py-2 px-3 text-gray-700 leading-tight bg-gray-200 border border-gray-200 rounded focus:outline-none focus:bg-white focus:border-gray-500">
                    <option value="">상세 상태 필터</option>
            ...
            <div class="flex items-center">
                <select name="hotelId" id="hotelId" wire:model.lazy="searchData.hotelId"
                    ...
                    <optgroup label="※ 메인 사이트 ------">
                        @foreach (\App\Hotel::whereStatus(2)->where('curator','=','N')->get() as $hotel)
                            <option value="{{$hotel->id}}">{{$hotel->option->title}}</option>
                        @endforeach
                    </optgroup>
                    <optgroup label="※ 큐레이터 ------">
                        @foreach (\App\Hotel::whereStatus(2)->where('curator','=','Y')->get() as $hotel)
                            <option value="{{$hotel->id}}">{{$hotel->option->title}}</option>
                        @endforeach
                    </optgroup>
            ...
            <div class="flex items-center">
                <input id="reservationOrder" name="reservationOrder" type="text"
                       wire:model.lazy='searchData.reservationOrder'
                       wire:change="searchDataSave"
            ...
            <div class="flex items-center">
                <div>
                    <livewire:admin.calculation.price :curatorVisible="$curatorVisible"></livewire:admin.calculation.price>
                </div>
            </div>
        ...
        @isset($reservations)
            <table class="min-w-full table-auto" wire:loading.remove>
                ...
                <tr class="bg-gray-700">
                    <th class="reservation_type py-2 px-2">
                        <div class="text-gray-300 whitespace-pre" wire:click.prevent="sortBy('type')">타입 @include('includes._sort-icon', ['field' => 'type'])</div>
                    </th>
                    <th class="order_name py-2 px-4">
                        <div class="text-gray-300 whitespace-pre" wire:click="sortBy('order_name')">입주자명 (변경성명) @include('includes._sort-icon', ['field' => 'order_name'])</div>
                    </th>

                    <th class="order_name py-2 px-4">
                        <div class="text-gray-300 whitespace-pre">결제자명</div>
                    </th>


                    <th class="order_email py-2">
                        <div class="text-gray-300 whitespace-pre">주문 번호</div>
                    </th>
                    ...
                    <th class="py-2 px-6">
                        <div class="text-gray-300 whitespace-pre">주문자 연락처</div>
                    </th>
                    ...
                    <th class="py-2 px-6">
                        <div class="text-gray-300 whitespace-pre">상품옵션</div>
                    </th>
                    <th class="py-2 px-24">
                    ...
                    <th class="py-2 px-6">
                        <div class="text-gray-300 whitespace-pre">추가결제금액</div>
                    </th>
                    <th class="py-2 px-6">
                        <div class="text-gray-300 whitespace-pre">취소금액</div>
                    </th>
                    <th class="py-2 px-6">
                ...
                @foreach ($reservations as $reservation)
                    <tr class="bg-white py-1 border border-t-2 border-b-2 border-gray-200 text-center">
                        <td class="reservation_type py-2">
                            <div class="whitespace-pre px-2">{{$reservation->Status}}</div>
                            @isset($reservation->curator)
                                <div class="whitespace-pre">큐레이터:{{$reservation->curator->name}}</div>
                            @endisset
                        </td>
                        <td class="order_name py-2 px-6">
                            {{$reservation->order_name ?? '정보없음'}}

                            @if(isset($reservation->user) && $reservation->order_name !== '' && $reservation->order_name !== $reservation->user->name)


                                <div>({{$reservation->user->name ?? '정보없음'}})</div>
                            @endif
                        </td>

                        <td class="order_name py-2">
                            {{$reservation->payment->name ?? '정보없음'}}
                        </td>

                        <td class="order_number py-2 px-8">
                            @if(isset($reservation->payment->order_id))
                                <div>{{\Illuminate\Support\Str::of($reservation->payment->order_id)->substr(-4)}}</div>
                            @else
                                <div>정보없음</div>
                            @endif
                        </td>
                        ...
                        <td class="order_email py-2 px-8">
                            <div>{{$reservation->order_email ?? '정보없음'}}</div>
                        </td>
                        <td class="order_hp py-2 px-4">
                            <div>{{$reservation->order_hp ?? '정보없음'}}</div>
                        </td>
                        <td class="hotel_data py-2 text-left">
                            {{ \App\HotelOption::whereHotelId($reservation->hotel->id ?? 0)->whereDisable('N')->first()->title ?? '호텔옵션없음' }}
                        </td>
                        ...
                        <td class="py-2">
                            <div>
                                ...
                                @if($reservation->type === 'month')
                                    <div>
                                        {{ $reservation->confirmations->count() >= 1 ?
                                            '입주 : '.$formatter->carbonFormat($reservation->confirmation->start_dt,'Y년 m월 d일(요일) H시 i분') : '' }}
                                    </div>
                                    <div>
                                        {{ $reservation->confirmations->count() >= 1 ?
                                            '퇴실 : '.$formatter->carbonFormat($reservation->confirmation->end_dt,'Y년 m월 d일(요일) H시 i분') : '' }}
                                    </div>
                            ...
                            <div class="pl-2 text-left space-y-1 w-max-content">
                                @if($reservation->type === 'month')
                                    <div class="bg-gray-200">
                                        확정룸
                                        : {!! $reservation->confirmation->room_type ?? '<span class="text-gray-400">정보없음</span>' !!}
                                    </div>
                        ...
                        <td class="py-2">
                            @if($reservation->payments->count() >= 1 && is_numeric($reservation->payment->total_price))
                                {{ number_format($reservation->payment->total_price) }}원
                            @endif
                        </td>
                        <td class="py-2">
                            @if($reservation->payments->count() >= 1 && is_numeric($reservation->payment->add_price))
                                {{ number_format($reservation->payment->add_price) }}원
                            @endif
                        </td>
                        <td class="py-2">
                            @if($reservation->payments->count() >= 1 && is_numeric($reservation->payment->cancel_price))
                                {{ number_format($reservation->payment->cancel_price) }}원
                            @endif
                        </td>
                        <td class="py-2">
                        ...
                        <td class="py-2">
                            <div class="px-2 w-max-content">
                                @if($reservation->type ==='month')
                                    @if($reservation->payments->count() >= 1 && $reservation->payment->order_completed_at !==null)
                        ...
                        <td class="payment_data py-2 text-center">
                            @if($reservation->payments->count()>=1)
                                @switch($reservation->payment->status)
                                    ...
                                    <div
                                        class="w-max-content px-4 mx-auto mt-1 text-white cursor-pointer py-1 bg-red-400 rounded-md hover:text-block hover:bg-red-600"
                        ...
                        <td class="confirmations_data">
                            <a class="cursor-pointer w-max-content px-4 mx-auto block bg-green-500 text-white px-1 py-2 border rounded-md hover:bg-green-700"
                               href="{{route('information.alertTalkList',['reservation_id'=>$reservation->id ?? null])}}">
                                ...
                                {{ \App\AlertTalkList::whereReservationId($reservation->id ?? null)->get()->count() }} 개
                            </a>
                        </td>
                        <td>
    ...
    <div class="px-2">
        ...
        @if(count($reservations)>1)
            <div class="w-full">
                <div class="py-2">
                    {{$reservations->links('vendor.livewire.tailwind')}}
                </div>
            </div>
        @endif
