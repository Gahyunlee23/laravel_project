        ...
        <table style="table-layout:auto;width: 100%;background-color: #ffffff;border:1px solid #7d7d7d;text-align: center;">
            ...
            <tr>
                <td colspan="3" style="border:1px solid #ededed;padding: 1rem 0.5rem;">
                    이전 기간 : {{$reservation->reservationModify->before_start_dt ?? '정보없음' }}
                    @if($reservation->isMonth())
                    ~ {{$reservation->reservationModify->before_end_dt ?? '정보없음'}}
                    @endif
                </td>
                <td colspan="3" style="border:1px solid #ededed;padding: 1rem 0.5rem;">
                    <div>
                        변경 희망 : {{$reservation->reservationModify->start_dt ?? '정보없음' }}
                        @if($reservation->isMonth())
                        ~ {{$reservation->reservationModify->end_dt ?? '정보없음'}}
                        @endif
                    </div>
                    <div>
                        변동 일수 : {{$reservation->reservationModify->diff_day ?? '정보없음' }}
                    </div>
                </td>
                <td colspan="3" style="border:1px solid #ededed;padding: 1rem 0.5rem;">
                    <div>
