...
//            'images.*' => ['image', 'size:1024', 'max:1024', 'mimes:png,jpg,jpeg'],
    ...
    public function updated($propertyName)
    {
        ...
        if(Str::of($propertyName)->contains(['img','Image','Img'])){
            $model = 'AddHotelImage';
            if(preg_match("/[\xE0-\xFF][\x80-\xFF][\x80-\xFF]/", $this->{$propertyName}->getClientOriginalName())){
                if(Str::of($propertyName)->contains('checkpoint')){
                    $path = 'entry/check-point/';
                    $model='AddHotelCheckPoint';
                }else{
    ...
    public function addHotelImageUpdateOrCreate($order, $image): void
    {
    ...
    public function addHotelCheckPointUpdateOrCreate($order, $image, $title, $explanation): void
    {
    ...
    public function addHotelImageDelete($order){
	...
	{
		return view('livewire.hotels.entry.hotel-images-and-check-points-edit');
	}
}
