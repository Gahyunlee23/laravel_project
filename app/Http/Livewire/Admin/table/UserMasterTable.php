...
namespace App\Http\Livewire\Admin\table;
    ...
    public function render()
    {
        if($this->reservation_id){
            $users=User::whereHas('reservations', function($query){
                    $query->whereId($this->reservation_id);
                }
            )->get();
        }else{
            $users=User::whereHas('reservations', function($query){
            ...
            )->where(function ($query) {/*User Start*/

                    if (isset($this->searchData['userData']) && $this->searchData['userData'] !== '' && $this->searchData['userData'] !== null) {
                        $query->where('name', 'like', '%' . $this->searchData['userData'] . '%');
                        $query->orwhere('email', 'like', '%' . $this->searchData['userData'] . '%');
                        $query->orwhere('tel', 'like', '%' . $this->searchData['userData'] . '%');
                    }

                }/*User End*/
            )->orderBy('id','DESC')
            ->orderBy('updated_at','DESC')
            ->paginate($this->searchData['reservationPagination'] ?? 10);

            //ddd($this->searchData['userData']);
        }

        $this->renderChange();

        return view('livewire.admin.information.user-master-table', [
            'users' => $users,
